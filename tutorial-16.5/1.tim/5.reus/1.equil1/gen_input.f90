   ...
   integer, parameter :: natom=11, ndim = 5, nw = 21
   integer :: an(natom), group(natom), rr_idx(2,ndim)
   real(8) :: rr(ndim,nw), fc(ndim)
   ...
   character(10)  :: id
     ...
     fc = 300.0  ! kcal/mol/Angs^2

     open(10,file='../0.window/win_rr.dat',status='old')
     do i = 1, nw
       read(10,*) j, rr(:,i)
     end do
     close(10)
 
     open(10,file='equil1_temp.inp', status='old')
     ...
     do i = 1, nw
       write(id,'(i0)') i
       open(11,file='equil1_'//trim(id)//'.inp',status='unknown')
       do j = 1, nsection
         if (index(section(1,j),"INPUT") > 0 .and. i > 1) then
           do nline = 1, nline_sec(j)
             k = index(section(nline, j),"rstfile")
             if (k > 0) then 
               write(11,'("rstfile = equil1_",i0,".rst")') i-1
             else
         ...
         if (index(section(1,j),"RESTR") > 0) then
           do k = 1, ndim
             ...
             write(11,'("constant",i0,"  = ",f8.1)') nfunction+k, fc(k) 
